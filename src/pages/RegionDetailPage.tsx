...
const RegionDetailPage: React.FC = () => {
  ...
  const [regionCreatures, setRegionCreatures] = useState([]);
  const [regionStories, setRegionStories] = useState([]);
  ...
  useEffect(() => {
    if (region) {
      ...
      const relatedCreatures = creatures.filter(c => c.origin === region.name);
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="relative h-96 rounded-lg overflow-hidden mb-12">
          ...
          <div className="absolute bottom-0 left-0 p-8">
            <h1 className="font-serif text-4xl font-bold text-white mb-4">{region.name}</h1>
        ...
        {regionCreatures.length === 0 && regionStories.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-400 text-lg">No content available for this region yet.</p>
          </div>
        )}
      </div>
    </div>
